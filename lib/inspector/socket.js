...
Socket.prototype.recvEvent = function (message) {
    ...
    this.logger.debug('[inspector#' + this.raw.id + '] recv ' + message);
    ...
    if (!(domainName in this.inspector)) {
        this.logger.error('[inspector#' + this.raw.id + '] the message is for unimplemented domain=' + domainName);
    ...
    if (!(functionName in dispatcher)) {
        this.logger.error('[inspector#' + this.raw.id + '] the message is for unimplemented method=' + messageObject.method);
...
Socket.prototype.send = function () {
    ...
    this.logger.debug('[inspector#' + this.raw.id + '] send ' + JSON.stringify(data));

    this.raw.emit('event', data);
};
