        ...
        function (socket) {
            ...
            socket.on(
                ...
                function (data, cb) {
                    ...
                    if (that.logger) {
                        that.logger.info('[dbgp-browser#' + socket.id + '] connected to session ' + data.sid);
                    }
            ...
            socket.sendInspectorMessage = function () {
                ...
                if (that.logger) {
                    that.logger.debug('[inspector#browser-' + socket.id + '] send ' + JSON.stringify(data));
                }
            ...
            if (that.logger) {
                that.logger.debug('[inspector#browser-' + socket.id + '] connected');

                socket.on(
                    'disconnect',
                    function () {
                        that.logger.debug('[inspector#browser-' + socket.id + '] disconnected');
                    }
                );
            }
        }
