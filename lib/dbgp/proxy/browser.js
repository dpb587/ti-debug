...
Browser.prototype.registerWebService = function (http, io) {
    ...
    http.get('/proxy.html', function (req, res) {
        ...
        res.send(view.standard(
            ...
            '<div class="row">' +
                '<div class="span2">&nbsp;</div>' +
                '<div id="dbgp-log" class="span10"><table class="table"><thead><tr><th colspan="2">Event Log</th></tr></thead><tbody></tbody></table></div>' +
            '</div>'
        ));
    });
...
Browser.prototype.recvAttach = function (socket, data, cb) {
    ...
    if (!that.sessions[data.sid]) {
        ...
        return;
    } else if (that.sessions[data.sid].attached) {
    ...
    socket.setInspector(Inspector.create(that.sessions[data.sid], socket));
    
    that.sessions[data.sid].on(
        'close',
        function () {
            socket.send(
