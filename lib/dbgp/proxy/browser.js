...
Browser.prototype.acceptConnection = function (socket) {
    ...
    that.logger.debug('[dbgp-browser#' + socket.id + '] connected');

    socket.on('proxyinit', that.recvProxyinit.bind(that, socket));
    socket.on('attach', that.recvAttach.bind(that, socket));
    socket.on(
        'disconnect',
        function () {
            that.logger.debug('[dbgp-browser#' + socket.id + '] disconnected');
        }
    );
};
...
Browser.prototype.recvProxyinit = function (socket, data, cb) {
    ...
    this.logger.info('[dbgp-browser#' + socket.id + '] registered idekey=' + data.idekey);
...
Browser.prototype.recvAttach = function (socket, data, cb) {
    if (!this.sessions[data.sid]) {
        this.logger.warn('[dbgp-browser#' + socket.id + '] session ' + data.sid + ' does not exist.');

        return;
    } else if (this.sessions[data.sid].attached) {
        this.logger.warn('[dbgp-browser#' + socket.id + '] session ' + data.sid + ' is already attached.');

        return;
    }

    this.logger.info('[dbgp-browser#' + socket.id + '] connected to session ' + data.sid);
