...
function Runtime(engine, browser)
{
    this.engine = engine;
...
Runtime.prototype.getProperties = function (params, callback) {
    if (params.objectId[0] == '|') {
        ...
        if (oid[1].substr(0, 3) == 'lvl') {
            if (oid.length == 3) {
                this.engine.send('context_get', { d : oid[1].substr(3), c : oid[2].substr(3) }, function (res) {
                    var data = res.property[0] ? res.property : [res.property];
                    var compiled = [];
                ...
                this.engine.send('property_get', { d : oid[1].substr(3), c : oid[2].substr(3), n : oid[3] }, function (res) {
                    var compiled = [];
