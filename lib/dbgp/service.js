...
Service.prototype.acceptConnection =  function (socket) {
    ...
    that.logger.info('[dbgp#' + socket.name + '] connected');

    socket.raw.on(
        'close',
        function () {
            that.logger.info('[dbgp#' + socket.name + '] disconnected');
        }
    );
    ...
    socket.once(
        ...
        function (data) {
            if (!that.proxy.initiate(this.header.init.idekey, this)) {
                that.logger.info('[dbgp#' + socket.name + '] no handler for idekey=' + this.header.init.idekey);
