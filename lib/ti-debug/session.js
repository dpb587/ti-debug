...
//
function Instance(id, backend, backendSocket, options, logger) {
    var that = this;
    ...
    that.timerConnectTimeout = setTimeout(
        function () {
            that.detach('Frontend did not connect within timeout.');
        },
        that.options.connect_timeout
    );

    // Register a listener to know if the connection is closed.
    this.backendSocket.on(
        'close',
        function () {
            that.backendSocketClosed = true;
...
Instance.prototype.attach = function (frontend, frontendSocket, callback) {
    ...
    this.frontendSocket.on(
        'close',
        function () {
            that.frontendSocketClosed = true;
...
Instance.prototype.detach = function (reason) {
    if (this.state != Instance_State.ATTACHED) {
        return;
    }
    ...
    if (this.logger) {
        this.logger.debug('session is detaching');
    }
};
...
Instance.prototype.attemptClose = function () {
    ...
    if (this.logger) {
        this.logger.debug('sessions is closed');
    }
